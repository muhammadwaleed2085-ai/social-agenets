...
export default function AdDraftsManager({ onRefresh }: AdDraftsManagerProps) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [draftsRes, campaignsRes, adSetsRes] = await Promise.all([
        fetch(`/api/v1/meta-ads/ads/draft?workspaceId=${workspaceId}`),
        fetch('/api/v1/meta-ads/campaigns'),
        fetch('/api/v1/meta-ads/adsets'),
      ]);
  ...
  const handleDelete = async (draftId: string) => {
    ...
    try {
      const response = await fetch(`/api/v1/meta-ads/ads/draft/${draftId}`, {
        method: 'DELETE',
      });
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // Update draft status
        await fetch(`/api/v1/meta-ads/ads/draft/${selectedDraft.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            status: 'published',
