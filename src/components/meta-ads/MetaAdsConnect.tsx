...
export default function MetaAdsConnect({ onConnect }: MetaAdsConnectProps) {
  ...
  const handleConnect = async () => {
    ...
    try {
      const response = await fetch('/api/v1/meta-ads/auth/url');
      const data = await response.json();
  ...
  // Meta Marketing API v25.0+ Features
  const features = [
    {
      icon: Target,
      title: 'Advanced Targeting',
      description: 'Reach ideal audiences with Advantage+ AI optimization',
    },
    {
      icon: BarChart3,
      title: 'Analytics',
    ...
    },
    {
      ...
      description: 'Create powerful audiences with lookalike targeting',
    },
    {
      icon: Zap,
      title: 'Advantage+ AI',
  ...
  const permissions = [
    'Manage ad campaigns (OUTCOME objectives)',
